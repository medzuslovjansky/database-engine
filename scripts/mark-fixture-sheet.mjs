...
async function main(targetLang) {
  ...
  for (const word of wordsList) {
    word.raw.sameInLanguages = word.sameInLanguages.toString();
  }

  await csv.writeFile(`__fixtures__/words.csv`, wordsList.map(w => w.raw));
  await updateSameInLanguages(wordsList.map(w => w.raw.sameInLanguages));
}

function updateIntelligibilityStatus(word, lang, record) {
  /** @type {core.IntelligibilityAssessment} */
  const assessment = word.sameInLanguages[lang];
...
async function parseNewInterslavicWordsList() {
  ...
  const words = rawWords.map((word) => {
    ...
    result.sameInLanguages = parse.sameInLanguages(word.sameInLanguages, false);
