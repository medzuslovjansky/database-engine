...
import * as D1Consts from '@auth-d1/consts';
import type { UserProfileDTO, UnitOfWork, UserProfileStager  } from '@interslavic/database-engine-auth';
...
describe('D1UserProfile Integration', () => {
  ...
  it('should create and find a user profile', async () => {
    ...
    // Verify user count
    const userCount = await ctx.userProfileQuery.getUserCount();
  ...
  it('should remove the existing user profile', async () => {
    ...
    // Verify user count is back to 0
    const userCount = await ctx.userProfileQuery.getUserCount();
