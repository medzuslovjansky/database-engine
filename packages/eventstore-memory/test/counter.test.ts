...
import type {AggregateRegistry, ProjectionRegistry} from '@interslavic/database-engine-eventstore';
import { AggregateRepository, ProjectionRunner } from '@interslavic/database-engine-eventstore';
import {
  InMemoryEventStore,
...
import {
  Counter,
  CounterSummaryProjection,
  createFreshAggregateRegistry,
  createFreshProjectionRegistry,
} from '@interslavic/database-engine-eventstore-memory/test';
...
describe('Counter e2e', () => {
  ...
  test('should create counter, increment, decrement, and project summary', async () => {
    ...
    await expect(repository.load<Counter>('counter/3')).resolves.toBeNull();
    await expect(repository.load<Counter>('counter/3', true)).rejects.toThrow('Aggregate not found');
