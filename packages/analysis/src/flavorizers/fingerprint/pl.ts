...
export const isv = () =>
  multireplacer
    ...
    .rule('[cćčtťdďđ]', (r) => r.regexp(/[cdtćčďđť]+/, ['t']))
    .rule('[sśšzžź]', (r) => r.regexp(/[szśšźž]+/, ['z']))
    .rule('[aåeěėęijoȯuųvy]', (r) => r.regexp(/[aeijouvyåėęěųȯ]+/, ['o']))
...
export const pl = () =>
  multireplacer
    ...
    .rule('^h → h/?', (r) => r.regexp(/^h/, ['h', '']))
    .rule('[cćčtdď]', (r) => r.regexp(/[cdtćčď]+/, ['t']))
    .rule('[sśšzźż]', (r) => r.regexp(/[szśšźż]+/, ['z']))
    .rule('[aąeěęijoóuwy]', (r) => r.regexp(/[aeijouwyóąęě]+/, ['o']))
